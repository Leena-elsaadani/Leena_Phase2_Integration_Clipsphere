...
function FieldError({ message }) {
  ...
  return (
    <p style={{ color: '#f87171', fontSize: '0.75rem', marginTop: '4px', marginBottom: 0, fontFamily: "'DM Sans', sans-serif" }}>
      {message}
    </p>
  );
}
...
export default function UploadPage() {
  ...
  // Form state — managed manually (JSX file, not TSX)
  const [title, setTitle]             = useState('');
  const [description, setDescription] = useState('');
  const [tags, setTags]               = useState('');
  const [visibility, setVisibility]   = useState('public');
  ...
  const handleFile = useCallback((f) => {
    if (!f) return;
    if (f.type !== "video/mp4") {
      setServerError("Only MP4 files are supported.");
      return;
    }
    setServerError(null);
    setFile(f);
  ...
  // ── validation ──────────────────────────────────────────────────────────────
  const validateForm = () => {
    const result = uploadSchema.safeParse({ title, description, visibility });
    if (result.success) {
      setFieldErrors({});
      return true;
    }
  ...
  const validateField = (field, value) => {
    const partial = { title, description, visibility, [field]: value };
    const result = uploadSchema.safeParse(partial);
    if (result.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!file) {
      setServerError("Please select a video file.");
      return;
    }
    ...
    <main className="min-h-screen bg-zinc-950 text-white">
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6">
          ...
          {previewURL && (
            <div className="rounded-xl overflow-hidden bg-zinc-900 border border-zinc-800">
              <p className="text-xs text-zinc-500 px-3 py-1.5 border-b border-zinc-800">Preview</p>
              {/* eslint-disable-next-line jsx-a11y/media-has-caption */}
          ...
          <div>
            ...
            <input
              ...
              maxLength={150}
              placeholder="Give your video a catchy title"
              className={`w-full bg-zinc-900 border rounded-xl px-4 py-2.5 text-white placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent ${
                fieldErrors.title ? 'border-red-500' : 'border-zinc-700'
              }`}
            />
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Tell viewers about your video…"
              className={`w-full bg-zinc-900 border rounded-xl px-4 py-2.5 text-white placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent resize-none ${
                fieldErrors.description ? 'border-red-500' : 'border-zinc-700'
              }`}
            />
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                validateField('visibility', e.target.value);
              }}
              className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-violet-500"
            >
              <option value="public">🌍 Public</option>
